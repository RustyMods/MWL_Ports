...
public class Port : MonoBehaviour, Interactable, Hoverable
{
    ...
    public void Awake()
    {
        ...
        m_view.Register<long>(nameof(RPC_RequestAccess), RPC_RequestAccess);
        m_view.Register<bool>(nameof(RPC_RequestResponse), RPC_RequestResponse);
    }
    ...
    public bool Interact(Humanoid user, bool hold, bool alt)
    {
        ...
        m_view.InvokeRPC(nameof(RPC_RequestAccess), playerID);
        m_currentHumanoid = user;
        return false;
    }

    public void RPC_RequestAccess(long sender, long playerID)
    {
        if (!m_view.IsOwner()) return;
        if (IsInUse())
        {
            m_view.InvokeRPC(nameof(RPC_RequestResponse), false);
        }
        else
        {
            ZDOMan.instance.ForceSendZDO(sender, m_view.GetZDO().m_uid);
            m_view.GetZDO().SetOwner(sender);
            m_view.InvokeRPC(nameof(RPC_RequestResponse), true);
        }
    }

    public void RPC_RequestResponse(long sender, bool granted)
    {
        if (!Player.m_localPlayer || PortUI.instance == null) return;
        if (granted)
        {
    ...
    private bool IsInUse() => m_inUse || m_containers.IsInUse();

    public void SetInUse(bool inUse)
    {
        if (!m_view.IsOwner() || inUse == m_inUse) return;
        m_inUse = inUse;
    }
